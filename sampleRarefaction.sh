...
if [ "$feature" = "none" ]; then    # not seperated by feature

    # Get sample ids from subject
    sample_id=$(mysql --defaults-extra-file=security.cnf -h clash.biomed.drexel.edu --database=$db_name -N -B -e "select samples.id from subjects right join samples on subjects.id = samples.subject_id where subjects.identifier='$subject'")
    # Get number of samples
    T=$(echo "$sample_id" | wc -l)
    sample_id=$(echo "${sample_id[@]}" | tr '\n' ',')
    sample_id=${sample_id::-1}
    # get ids of qualified clones with size >= size_threshold
    qualified_clones=$(mysql --defaults-extra-file=security.cnf -h clash.biomed.drexel.edu --database=$db_name -N -B -e "select clone_id from sequences where sample_id in ($sample_id) and clone_id is not NULL and functional=1 group by clone_id having count(distinct seq_id)>=$size_threshold")
    # output the qualified clone id to file
    output_file="$subject-$feature-C$size_threshold-clones.tsv"
    echo "$qualified_clones" > $output_file
    count=$(echo "$qualified_clones" | wc -l)
    echo "$count clones with at least $size_threshold instance(s)"
    # replace \n with , for mysql command
    ...
    # Rename output
    output_file="$subject-$feature-C$size_threshold-addiontalSamples.tsv"
    mv addtionalSamples.tsv $output_file
    rm parameters.tsv

else    # calculate based on feature
